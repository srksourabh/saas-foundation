# Scaffolding script — <project-name>
...
import { create${Name}Schema, update${Name}Schema } from "@validators/${Name}";
import { db } from "@db";

export const ${Name}Router = createTRPCRouter({
  list: protectedProcedure.query(async () => {
    return await db.select().from(${Name}).orderBy(${Name}.createdAt);
  }),

  getById: protectedProcedure.input(z.string().uuid()).query(async ({ input }) => {
    const result = await db.select().from(${Name}).where(eq(${Name}.id, input)).limit(1);
    return result[0];
  }),

  create: protectedProcedure.input(create${Name}Schema).mutation(async ({ input, ctx }) => {
    const result = await db.insert(${Name}).values(input).returning();
    return result[0];
  }),
  ...
  update: protectedProcedure.input(z.object({
    ...
    const result = await db.update(${Name}).set(input.data).where(eq(${Name}.id, input.id)).returning();
    return result[0];
  }),

  delete: protectedProcedure.input(z.string().uuid()).mutation(async ({ input }) => {
    await db.delete(${Name}).where(eq(${Name}.id, input));
    return { success: true };
  }),
});
"@
    ...
    Write-Host "  1. Add columns to the schema"
...
switch ($Type) {
    ...
    "feature" {
        ...
        New-Model $Name
        New-Page $Name
        Write-Host "Feature '$Name' created. Generate tests with: pnpm scaffold test $Name" -ForegroundColor Green
    }
    "email" {
